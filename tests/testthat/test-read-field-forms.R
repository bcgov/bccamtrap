...
test_that("read_deployments_csv", {
  f <- test_path("deployments-field-form.csv")

  ret <- read_deployments_csv(f)
  expect_s3_class(ret, c("deployments_info", "sf"))

  ret2 <- read_deployments_csv(f, as_sf = FALSE)
  expect_false("sf" %in% class(ret2))
})

test_that("read_deployments_csv is like read_deployments", {
  f <- test_path("deployments-field-form.csv")

  ret <- read_deployments_csv(f)
  ssi1 <- make_deployments(test_meta_file1)
