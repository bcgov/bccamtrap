...
function to run some basic spatial validation on the data - namely
...
Use the [`summary()`](https://rdrr.io/r/base/summary.html) method for
Sample Station Info for basic descriptive stats:
...
function to create an interactive map the of the stations. This will
show any potential outlying stations, indicating possible data errors:
...
![](reference/figures/README-map-stations-1.png)

#### Camera Information:

Read camera information using
...
There is a handy [`summary()`](https://rdrr.io/r/base/summary.html)
method for this as well:
...
![](reference/figures/README-mapview-deployments-1.png)
...
Again, we can use the [`summary()`](https://rdrr.io/r/base/summary.html)
method to get an overview of the image data.
...
function to find deployment labels that are in the deployment data but
not in the image data, and vice-versa. It is usually likely that there
will be deployment labels in the deployment data that are missing from
the image data if not all of the images have been processed yet.
Deployment labels that are present in the image data but not in the
deployment data indicate a potential problem.
...
to join the deployment metadata to the images:
...
function to detect the following problems:
...
![](reference/figures/README-plot-deployments-1.png)
...
![](reference/figures/README-plot-snow-1.png)
...
![](reference/figures/README-plot-detections-1.png)
...
![](reference/figures/README-plot-diel-1.png)
...
By default, it calculates RAI per species using the sample start and end
dates in the data for each deployment:
...
similar to
[`make_sample_sessions()`](https://bcgov.github.io/bccamtrap/reference/make_sample_sessions.md),
set custom session start and end dates:
...
![](reference/figures/README-sample-rai4-plot-1.png)
...
![](reference/figures/README-sample-rai-5-1.png)

#### Relative Activity Index (RAI) over time

Use
[`rai_by_time()`](https://bcgov.github.io/bccamtrap/reference/rai_by_time.md)
to calculate RAI over a time window, optionally calculating statistics
using a moving window aggregation. You can calculate daily statistics,
or aggregate by week, month, or year. By default, it calculates daily
metrics, aggregating across deployments.
...
![](reference/figures/README-rai-by-time2-1.png)
...
![](reference/figures/README-rai-by-time3-1.png)
...
![](reference/figures/README-rai-by-time4-1.png)
...
to write all of the data to a SPI template, filling in just the default
required fields. This will fill in all of the tabs except for the
Project Info sheet which you must fill in manually.
...
function, passing in both the `sample_station_info` and `deployments`,
as well as the `image_data`.
...
change the `max_upload_size_mb` parameter of
[`bccamtrapp()`](https://bcgov.github.io/bccamtrap/reference/bccamtrapp.md)
to a value greater than the default of 50:
...
[CONTRIBUTING](https://bcgov.github.io/bccamtrap/CONTRIBUTING.md)
guidelines.

Please note that this project is released with a [Contributor Code of
Conduct](https://bcgov.github.io/bccamtrap/CODE_OF_CONDUCT.md). By
